...
def parse_args():
    ...
    parser.add_argument("--log_wandb", action='store_true', help="Whether to log training process to wandb", default=True)
    parser.add_argument("--dataset", type=str, required=True)
    parser.add_argument("--data_path", type=str, required=True)
    parser.add_argument("--dataset_type", type=str, required=True, choices=["classification", 'regression'])
    parser.add_argument("--metric", type=str, required=True, choices=['rocauc', 'ap', 'acc', 'rmse', 'mae', 'r2', 'spearman', 'pearson','cls-all'])
    ...
    parser.add_argument("--use_cpu", action='store_true', help="Force use CPU instead of GPU")
...
def finetune(args):
    ...
    # 根据设备类型选择合适的加载方式
    if device.type == 'cpu':
        # CPU模式下需要指定map_location来加载GPU训练的模型
        model_state = torch.load(f'{args.model_path}', map_location='cpu')
    else:
        model_state = torch.load(f'{args.model_path}')
