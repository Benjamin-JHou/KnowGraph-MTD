...
if __name__ == '__main__':
    ...
    # 根据是否使用CPU选择不同的DataLoader设置
    if args.use_cpu or not torch.cuda.is_available():
        train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], num_workers=args.n_threads, worker_init_fn=seed_worker, drop_last=True, collate_fn=collator, shuffle=True)
    else:
        train_loader = DataLoader(train_dataset, sampler=DistributedSampler(train_dataset), batch_size=config['batch_size'], num_workers=args.n_threads, worker_init_fn=seed_worker, drop_last=True, collate_fn=collator)
    model = LiGhT(
    ...
    # 根据是否使用CPU选择是否使用分布式训练
    if not (args.use_cpu or not torch.cuda.is_available()):
        # 如果使用GPU且支持分布式，则使用DistributedDataParallel
        try:
            ...
            # 如果分布式初始化失败，使用单GPU训练
            local_rank = 0
            print("Distributed training failed, using single GPU")
    else:
        local_rank = 0
    ...
    # 只有在使用分布式训练时才销毁进程组
    if not (args.use_cpu or not torch.cuda.is_available()) and torch.distributed.is_initialized():
