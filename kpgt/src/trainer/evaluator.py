...
class Evaluator:
    ...
    def eval(self, y_true, y_pred, valid_ids=None, direct=False):
        y_true, y_pred, valid_ids = self._parse_and_check_input(y_true, y_pred, valid_ids)
        if direct:
            return y_pred, y_true
        if self.eval_metric == 'cls-all':
            return {'rocauc': self._eval_rocauc(y_true, y_pred), 
                    'ap': self._eval_ap(y_true, y_pred), 
